...
In the current study, we evaluated logistic regression (LR), support vector machine (SVM) and random forest (RF) for the prediction of AMR for the antibiotics ciprofloxacin (CIP), cefotaxime (CTX), ceftazidime (CTZ), and gentamicin (GEN) based on WGS data with label encoding and FCGR encoding.
